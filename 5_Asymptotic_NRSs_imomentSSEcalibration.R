...
simulatedbatch_bias_Monte<-foreach(batchnumber =c((1:length(allkurtWeibull))), .combine = 'rbind') %dopar% {
  ...
  write.csv(SEbataches2,paste("asymptotic_Weibull_Imomentscalibration_raw_SWA",samplesize,round(kurtx,digits = 1),".csv", sep = ","), row.names = FALSE)
...
simulatedbatch_bias_Monte_SE<-foreach(batchnumber =c((1:length(allkurtWeibull))), .combine = 'rbind') %dopar% {
  ...
  SEbataches<- read.csv(paste("asymptotic_Weibull_Imomentscalibration_raw_SWA",samplesize,round(kurtx,digits = 1),".csv", sep = ","))
...
write.csv(simulatedbatch_bias_Monte_SE,paste("asymptotic_Weibull_Imomentscalibration_raw_SWA_error",largesize,".csv", sep = ","), row.names = FALSE)
