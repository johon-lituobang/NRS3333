...
simulatedbatch_bias_Monte<-foreach(batchnumber =c((1:length(allkurtWeibull))), .combine = 'rbind') %dopar% {
  ...
  for (batch1 in c(1:batchsize)){
    ...
    rqsmoments1<-rqsmoments(x=sortedx,dtype1=1,releaseall=TRUE,standist_d=d_values,orderlist1_sorted20=orderlist1_AB20,orderlist1_sorted30=orderlist1_AB30,orderlist1_sorted40=orderlist1_AB40,orderlist1_hlsmall=orderlist1_hlsmall,orderlist1_hllarge=orderlist1_hllarge,percentage=1/24,batch="auto",stepsize=1000,criterion=criterionset,boot=TRUE)
    rqsmoments1<-unlist(rqsmoments1)
  ...
  write.csv(RMSEbataches,paste("finite_Weibull_I2calibration_raw",samplesize,round(kurtx,digits = 1),".csv", sep = ","), row.names = FALSE)
...
write.csv(simulatedbatch_bias_Monte,paste("finite_Weibull_I2calibration_raw",samplesize,".csv", sep = ","), row.names = FALSE)

simulatedbatch_bias_Monte<- read.csv(paste("finite_Weibull_I2calibration_raw",samplesize,".csv", sep = ","))

Optimum_RMSE<-simulatedbatch_bias_Monte[,1:1772]

write.csv(Optimum_RMSE,paste("finite_I2_Weibull.csv", sep = ","), row.names = FALSE)
...
simulatedbatch_bias_Monte_SE<-foreach(batchnumber =c((1:length(allkurtWeibull))), .combine = 'rbind') %dopar% {
  ...
  SEbataches<- read.csv(paste("finite_Weibull_I2calibration_raw",samplesize,round(kurtx,digits = 1),".csv", sep = ","))
...
write.csv(simulatedbatch_bias_Monte_SE,paste("finite_Weibull_I2calibration_raw_error",samplesize,".csv", sep = ","), row.names = FALSE)


finite_I_Weibull<- read.csv(("finite_I2_Weibull.csv"))


finite_I_Weibull<-data.frame(finite_I_Weibull)

asymptotic_I_Weibull<- read.csv(paste("asymptotic_I2.csv", sep = ","))
colnames(finite_I_Weibull)<-colnames(asymptotic_I_Weibull)
all1<-rbind(asymptotic_I_Weibull,finite_I_Weibull)

write.csv(all1,paste("I2_values.csv", sep = ","), row.names = FALSE)
