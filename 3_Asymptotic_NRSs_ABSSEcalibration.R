...
simulatedbatch_bias_Monte<-foreach(batchnumber =c((1:length(allkurtWeibull))), .combine = 'rbind') %dopar% {
  ...
  for (batch1 in c(1:batchsize)){
    ...
    x<-c()
    rqmoments1<-rqmoments(x=sortedx,dtype1=1,releaseall=TRUE,standist_d=d_values,orderlist1_sorted2=orderlist1_AB2,orderlist1_sorted3=orderlist1_AB3,orderlist1_sorted4=orderlist1_AB4,interval=8,batch="auto",stepsize=1000,criterion=criterionset)
...
Label_ABSE_Weibull1<- read.csv(("I_Label.csv"))

Optimum_ABSE<-cbind(simulatedbatch_bias_Monte[,1:4],simulatedbatch_bias_Monte[,628:ncol(simulatedbatch_bias_Monte)])

colnames(Optimum_ABSE)<-colnames(Label_ABSE_Weibull1)

write.csv(Optimum_ABSE,paste("asymptotic_I_Weibull_SWA.csv", sep = ","), row.names = FALSE)
