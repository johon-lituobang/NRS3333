...
simulatedbatch_bias_Monte<-foreach(batchnumber =c((1:length(allkurtlognorm))), .combine = 'rbind') %dopar% {
  ...
  rqmean<-sqrt(colMeans((RMSEbataches3[1:batchsize,c(generate_indices(start=3, end=192*30-1+3-27-39-1-51-1-71-1, step = 192,step2=71))]-targetm)^2))
  
  rqvar<-sqrt(colMeans((RMSEbataches3[1:batchsize,c(generate_indices(start=75, end=192*30-1+3-27-39-1-51-1, step = 192,step2=51))]-targetvar)^2))
  
  rqtm<-sqrt(colMeans((RMSEbataches3[1:batchsize,c(generate_indices(start=127, end=192*30-1+3-27-39-1, step = 192,step2=39))]-targettm)^2))
  
  rqfm<-sqrt(colMeans((RMSEbataches3[1:batchsize,c(generate_indices(start=167, end=192*30-1+3-27, step = 192,step2=27))]-targetfm)^2))
...
Optimum_RMSE<-simulatedbatch_bias_Monte[,1:(192*30-1+5)]
...
simulatedbatch_bias_Monte_SE<-foreach(batchnumber =c((1:length(allkurtlognorm))), .combine = 'rbind') %dopar% {
  ...
  rqmean_se<-apply(((SEbataches[1:batchsize,c(generate_indices(start=3, end=192*30-1+3-27-39-1-51-1-71-1, step = 192,step2=71))])), 2, se_sd)
  
  rqvar_se<-apply((SEbataches[1:batchsize,c(generate_indices(start=75, end=192*30-1+3-27-39-1-51-1, step = 192,step2=51))]), 2, se_sd)
  
  rqtm_se<-apply((SEbataches[1:batchsize,c(generate_indices(start=127, end=192*30-1+3-27-39-1, step = 192,step2=39))]), 2, se_sd)
  
  rqfm_se<-apply((SEbataches[1:batchsize,c(generate_indices(start=167, end=192*30-1+3-27, step = 192,step2=27))]), 2, se_sd)
