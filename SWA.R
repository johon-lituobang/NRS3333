...
rqm<-function(x,sorted=FALSE){
  ...
  mmm1raw<-SWA8(x=x_ordered,interval=8,batch="auto",sorted = TRUE)
  
  mmm1_BM_rm_exp1<-mmmprocessrm(SWA=mmm1raw[2],median=mmm1raw[8],drm=0.37523)
  
  mmm1_BM_qm_exp1<-mmmprocessqm(x=x_ordered,percentage=1/8,pc1=CDF(x=x_ordered, xevaluated =mmm1raw[2], sorted = TRUE),dqm=0.32128)
