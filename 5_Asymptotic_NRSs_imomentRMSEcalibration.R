...
simulatedbatch_bias_Monte<-foreach(batchnumber =c((1:length(allkurtWeibull))), .combine = 'rbind') %dopar% {
  ...
  rqvar<-apply((SEbataches2[1:batchsize,c(73:120)]), 2, calculate_column_sd)
  
  rqtm<-apply((SEbataches2[1:batchsize,c(121:156)]), 2, calculate_column_sd)
...
Optimum_SE<-simulatedbatch_bias_Monte[,1:184]
...
simulatedbatch_bias_Monte_SE<-foreach(batchnumber =c((1:length(allkurtWeibull))), .combine = 'rbind') %dopar% {
  ...
  rqvar_se<-apply((SEbataches[1:batchsize,c(73:120)]), 2, se_sd)
  
  rqtm_se<-apply((SEbataches[1:batchsize,c(121:156)]), 2, se_sd)
  
  rqfm_se<-apply((SEbataches[1:batchsize,c(157:180)]), 2, se_sd)
