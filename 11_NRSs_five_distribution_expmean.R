...
simulatedbatch_asymptoticbias<-foreach(batchnumber = (1:length(allkurtWeibull)), .combine = 'rbind') %dopar% {
  ...
  rqmean_exp1<-rqmean_exp(x=sortedx,standist_d=d_values,orderlist1_sorted20=orderlist1_AB2_asymptotic,orderlist1_sorted30=orderlist1_AB3_asymptotic,orderlist1_sorted40=orderlist1_AB4_asymptotic,percentage=1/16,batch="auto",stepsize=1000,criterion=1e-30,boot=TRUE)
  rqmean_exp1<-unlist(rqmean_exp1)
  momentsx<-unbiasedmoments(x=sortedx)
  
  sortedx<-c()
  momentssd<-c(sd=sqrt(momentsx[2]))
  
  allrawmoBias<-c(
    firstbias=abs(c(Huberx,SMWM9,(rqmean_exp1))-targetm)/momentssd)
  allrawmo1<-c(first=c(Huberx,SMWM9,(rqmean_exp1))
  )
  all1<-(c(kurtx,skewx,momentsx,allrawmoBias,momentssd,allrawmo1,Huberx,SMWM9,rqmean_exp1,targetall))
}

write.csv(simulatedbatch_asymptoticbias,paste("asymptotic_Weibull_raw_Process_meanexp",largesize,".csv", sep = ","), row.names = FALSE)

write.csv(cbind(simulatedbatch_asymptoticbias[1:length(allkurtWeibull),1],simulatedbatch_asymptoticbias[1:length(allkurtWeibull),c(1:103)]),paste("asymptotic_Weibull_meanexp",largesize,".csv", sep = ","), row.names = FALSE)
...
simulatedbatch_asymptoticbias<-foreach(batchnumber = (1:length(allkurtgamma)), .combine = 'rbind') %dopar% {
  ...
  allrawmoBias<-c(
    firstbias=abs(c(Huberx,SMWM9,rqmean_exp1)-targetm)/momentssd[1])
  allrawmo1<-c(first=c(Huberx,SMWM9,rqmean_exp1)
  )
  all1<-(c(kurtx,skewx,momentsx,allrawmoBias,momentssd,allrawmo1,Huberx,SMWM9,rqmean_exp1,targetall))
}

write.csv(simulatedbatch_asymptoticbias,paste("asymptotic_gamma_raw_Process_meanexp",largesize,".csv", sep = ","), row.names = FALSE)

write.csv(cbind(simulatedbatch_asymptoticbias[1:length(allkurtgamma),1],simulatedbatch_asymptoticbias[1:length(allkurtgamma),c(1:103)]),paste("asymptotic_gamma_meanexp",largesize,".csv", sep = ","), row.names = FALSE)
...
write.csv(simulatedbatch_asymptoticbias,paste("asymptotic_Pareto_raw_Process_meanexp",largesize,".csv", sep = ","), row.names = FALSE)

write.csv(cbind(simulatedbatch_asymptoticbias[1:length(allkurtPareto),1],simulatedbatch_asymptoticbias[1:length(allkurtPareto),1:103]),paste("asymptotic_Pareto_meanexp",largesize,".csv", sep = ","), row.names = FALSE)
...
write.csv(simulatedbatch_asymptoticbias,paste("asymptotic_lognorm_raw_Process_meanexp",largesize,".csv", sep = ","), row.names = FALSE)

write.csv(cbind(simulatedbatch_asymptoticbias[1:length(allkurtlognorm),1],simulatedbatch_asymptoticbias[1:length(allkurtlognorm),1:103]),paste("asymptotic_lognorm_meanexp",largesize,".csv", sep = ","), row.names = FALSE)
...
write.csv(simulatedbatch_asymptoticbias,paste("asymptotic_gnorm_raw_Process_meanexp",largesize,".csv", sep = ","), row.names = FALSE)

write.csv(cbind(simulatedbatch_asymptoticbias[1:length(allkurtgnorm),1],simulatedbatch_asymptoticbias[1:length(allkurtgnorm),1:103]),paste("asymptotic_gnorm_meanexp",largesize,".csv", sep = ","), row.names = FALSE)
