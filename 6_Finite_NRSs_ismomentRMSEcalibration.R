...
simulatedbatch_bias_Monte<-foreach(batchnumber =c((1:length(allkurtWeibull))), .combine = 'rbind') %dopar% {
  ...
  write.csv(SEbataches2,paste("finite_Weibull_Ismomentscalibration_raw",samplesize,round(kurtx,digits = 1),".csv", sep = ","), row.names = FALSE)
...
simulatedbatch_bias_Monte_SE<-foreach(batchnumber =c((1:length(allkurtWeibull))), .combine = 'rbind') %dopar% {
  ...
  SEbataches<- read.csv(paste("finite_Weibull_Ismomentscalibration_raw",samplesize,round(kurtx,digits = 1),".csv", sep = ","))
...
write.csv(simulatedbatch_bias_Monte_SE,paste("finite_Weibull_Ismomentscalibration_raw_error",samplesize,".csv", sep = ","), row.names = FALSE)

finite_I_Weibull<- read.csv(("finite_Ismoments_Weibull.csv"))


finite_I_Weibull<-data.frame(finite_I_Weibull)

asymptotic_I_Weibull<- read.csv(paste("asymptotic_Ismoments.csv", sep = ","))
