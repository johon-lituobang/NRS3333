...
Monte_Two<-simulatedbatch_bias_Monte[,c(1,2,3,4,545:ncol(simulatedbatch_bias_Monte))]
...
Monte_d_Two<-cbind(meanall[,1:4],compalldmoments1[,c(seq(from=1, to=540, by=3))])
...
Allstandarderror_each1<-Allstandarderror_each[,545:ncol(Allstandarderror_each)]
...
Monte_d_Two_error<-cbind(Allstandarderror_each[,1:4],compdall_error[,c(seq(from=1, to=540, by=3))])
