...
simulatedbatch_bias_Monte<-foreach(batchnumber =c((1:length(allkurtWeibull))), .combine = 'rbind') %dopar% {
  ...
  write.csv(RMSEbataches,paste("asymptotic_Weibull_I2calibration_raw",samplesize,round(kurtx,digits = 1),".csv", sep = ","), row.names = FALSE)
...
write.csv(simulatedbatch_bias_Monte,paste("asymptotic_Weibull_I2calibration_raw",largesize,".csv", sep = ","), row.names = FALSE)

simulatedbatch_bias_Monte<- read.csv(paste("asymptotic_Weibull_I2calibration_raw",largesize,".csv", sep = ","))

Optimum_RMSE<-simulatedbatch_bias_Monte[,1:1772]

write.csv(Optimum_RMSE,paste("asymptotic_I2_Weibull.csv", sep = ","), row.names = FALSE)
...
simulatedbatch_bias_Monte_SE<-foreach(batchnumber =c((1:length(allkurtWeibull))), .combine = 'rbind') %dopar% {
  ...
  SEbataches<- read.csv(paste("asymptotic_Weibull_I2calibration_raw",samplesize,round(kurtx,digits = 1),".csv", sep = ","))
...
write.csv(simulatedbatch_bias_Monte_SE,paste("asymptotic_Weibull_I2calibration_raw_error",largesize,".csv", sep = ","), row.names = FALSE)

asymptotic_I_Weibull<- read.csv(("asymptotic_I2_Weibull.csv"))


write.csv(asymptotic_I_Weibull,paste("asymptotic_I2.csv", sep = ","), row.names = FALSE)
